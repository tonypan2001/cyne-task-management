...
import { teamService, TeamMember } from "@/services/teamService"; // ✨ Import Service และ Type
...
export default function TeamPage() {
  ...
  const handleRemoveClick = (member: TeamMember) => {
    if (member.role === "owner") {
      showToast(
        "Permission Denied",
        "warning",
        "ไม่สามารถลบ Owner ออกจากพื้นที่ทำงานได้",
      );
      return;
    }
  ...
  const confirmRemoveMember = async () => {
    ...
    try {
      // ✨ เรียกใช้ Service สำหรับลบสมาชิก
      await teamService.removeMember(memberToRemove.member_id);

      setMembers((prev) =>
        prev.filter((m) => m.member_id !== memberToRemove.member_id),
      );
      showToast(
        "Member Removed",
        "success",
        `นำ ${memberToRemove.email} ออกจากทีมเรียบร้อยแล้ว`,
      );
        ...
        <div className="divide-y divide-slate-50">
          {members.map((member) => (
            <div
              ...
              className="flex flex-col sm:flex-row sm:items-center justify-between p-8 md:px-10 gap-4 hover:bg-slate-50/50 transition-colors group"
            >
