...
export const taskService = {
    ...
    uploadImage: async (userId: string, file: File) => {
        ...
        if (error) throw error
        return supabase.storage.from('task-images').getPublicUrl(fileName).data.publicUrl
    },

    // 4. สร้างงานใหม่ (พร้อม Sub-tasks)
    createTask: async (taskData: Partial<Task>, subTasks: { title: string }[]) => {
        const supabase = createClient()
    ...
    updateTask: async (taskId: string, updateData: Partial<Task>, subTasks: { title: string }[]) => {
        const supabase = createClient()
